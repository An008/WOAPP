...
function renderHistory(){
  ...
  document.getElementById('v-history').innerHTML='<div class="hdr"><div class="hdr-ttl">History</div><div class="hdr-sub">'+keys.length+' sessions</div></div>'+keys.map(function(d){var sess=S.sessions[d],sd=SESSIONS[sess.type],comp=sessComp(d);var dt=sess.date||d.split('|')[0];var c=cols[sess.type]||'var(--txt2)';var pc=comp.pct>=100?'var(--green)':comp.pct>0?'var(--amber)':'var(--txt3)';return '<div class="hi" onclick="beginSession(\''+sess.type+'\',\''+(sess.date||d.split('|')[0])+'\')"><div class="hi-d" style="background:'+c+'"></div><div class="hi-dt">'+dt+'</div><div style="flex:1"><div style="font-size:14px;font-weight:700;color:var(--white)">'+(sd?sd.name:sess.type)+'</div><div style="font-size:11px;color:var(--txt2)">'+(sd?sd.dur:'')+'</div></div><div style="font-size:13px;font-weight:800;color:'+pc+'">'+comp.pct+'%</div></div>';}).join('');
}
